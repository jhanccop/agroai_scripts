...
bool isJoined = false; 
uint32_t sleepCycleMs = 300000;
bool sleepCycleLoaded = false;
...
/* =============== FUNCIÓN PARA LEER ACELERÓMETRO SEGÚN EJE =============== */
float readAccelerometer() {
  sensors_event_t a, g, temp;
  mpu.getEvent(&a, &g, &temp);
  
  if (accelerometerAxis == "x") {
    return a.acceleration.x;
  } else if (accelerometerAxis == "y") {
    return a.acceleration.y;
  } else if (accelerometerAxis == "z") {
...
void loadAllConfigurations() {
  ...
  accelerometerAxis = readFile(SPIFFS, "/acc_axis.txt");
  if (accelerometerAxis == "") accelerometerAxis = "x";
...
void loadSleepCycleOnce() {
  ...
  if (sleepStr != "" && sleepStr.length() > 0) {
    ...
    if (sleepSeconds >= 10 && sleepSeconds <= 86400) {
      sleepCycleMs = sleepSeconds * 1000;
      sleepCycleLoaded = true;
      Serial.println("✓ Sleep cycle loaded from SPIFFS: " + String(sleepSeconds) + "s");
      return;
    }
  }
  ...
  Serial.println("✓ Using default sleep cycle: 300s");
}
...
void wifiTx() {
  ...
  while (WiFi.status() != WL_CONNECTED && (millis() - wifiStartTime) < wifiTimeout) {
    delay(500);
    attempts++;
    oledShow("WiFi", "Connecting...", "Attempt: " + String(attempts), "Time: " + String((millis() - wifiStartTime)/1000) + "s");
  }

  if (WiFi.status() != WL_CONNECTED) {
    oledShow("WiFi Failed!", "Entering", "Deep Sleep", "30s timeout");
    Serial.println("❌ WiFi connection failed, entering deep sleep");
    ledBlink('R', 3);
    
    prepararDeepSleep();
    esp_sleep_enable_timer_wakeup(30000 * 1000);
...
void displaySimpleGraphs(Average<uint16_t>& rLoad, Average<float>& rAccel, uint16_t ndata) {
  ...
  const uint8_t GRAPH_WIDTH = 95;
  const uint8_t STATS_X = 98;
...
float calLenght(Average<float>& rAccel,int i_start,int i_end,float sample_time_ms) {
  ...
  const float GRAVEDAD = 9810.0;
  float dt = sample_time_ms / 1000.0;
  
  float velocidad = 0.0;
  float posicion = 0.0;
  float altura_maxima = 0.0;
...
void mainTask() {
  ...
  Average<float> rAccel(nData);

  sensors_event_t a, g, temp;
  mpu.getEvent(&a, &g, &temp);
  float fAcc = readAccelerometer();
  ...
  for (uint16_t i = 0; i < 50; i++) {
    ...
    float accel_val = readAccelerometer();
    fAcc = ALPHA_A * accel_val + (1.0 - ALPHA_A) * fAcc;
    blAcc += fAcc;
  ...
  float accelStdDev = rAccel.stddev();
  int maxIdx, minIdx;
  float maxAccel = rAccel.maximum(&maxIdx);
  float minAccel = rAccel.minimum(&minIdx);
  ...
  spm = 60000.0 / (cycleSamples * meanRead);
...
void handleRoot() {
  ...
  if (currentSensorType == "") currentSensorType = "1";
  ...
  // Accelerometer Axis Configuration Section
  html += "<div class='section'><h2>📊 Accelerometer Axis Configuration</h2>";
  html += "<div class='alert alert-info'>Select the measurement axis for the accelerometer</div>";
  html += "<form id='axisForm'>";
  html += "<div class='checkbox-group'>";
  
  String currentAxis = readFile(SPIFFS, "/acc_axis.txt");
  if (currentAxis == "") currentAxis = "x";
  
  String xChecked = (currentAxis == "x") ? "checked" : "";
  String yChecked = (currentAxis == "y") ? "checked" : "";
  String zChecked = (currentAxis == "z") ? "checked" : "";
  
  html += "<input type='radio' id='axisX' name='accAxis' value='x' " + xChecked + ">";
  html += "<label for='axisX'>X Axis</label>";
  html += "</div>";
  html += "<div class='checkbox-group'>";
  html += "<input type='radio' id='axisY' name='accAxis' value='y' " + yChecked + ">";
  html += "<label for='axisY'>Y Axis</label>";
  html += "</div>";
  html += "<div class='checkbox-group'>";
  html += "<input type='radio' id='axisZ' name='accAxis' value='z' " + zChecked + ">";
  html += "<label for='axisZ'>Z Axis</label>";
  html += "</div>";
  html += "<button type='button' class='btn btn-success btn-block' onclick='saveAccelerometerAxis()'>💾 Save Axis</button>";
  html += "</form></div>";
  ...
  // Accelerometer Axis save function
  html += "function saveAccelerometerAxis(){";
  html += "let axis=document.querySelector('input[name=\"accAxis\"]:checked');";
  html += "if(!axis){alert('Please select an axis');return;}";
  html += "fetch('/setAccelerometerAxis?axis='+axis.value)";
  html += ".then(r=>r.text()).then(msg=>alert(msg)).catch(e=>alert('Error: '+e))}";
...
void handleSetAccelerometerAxis() {
  if (server.hasArg("axis")) {
    String axis = server.arg("axis");
    if (axis == "x" || axis == "y" || axis == "z") {
      saveSetting(axis, "/acc_axis.txt");
      accelerometerAxis = axis;
      
      oledShow("Accelerometer", "Axis Updated!", "Axis: " + axis, "Saved!");
      ledBlink('G', 2);
      
      server.send(200, "text/plain", "Accelerometer axis saved: " + axis);
    } else {
      server.send(400, "text/plain", "Invalid axis. Must be x, y, or z");
    }
  } else {
...
void handleSetSleep() {
  if (server.hasArg("cycle")) {
    ...
    if (cycleSeconds >= 10 && cycleSeconds <= 86400) {
      ...
      Serial.println("✓ Sleep cycle updated: " + String(cycleSeconds) + "s");

      server.send(200, "text/plain",
                  "✓ Sleep cycle saved: " + String(cycleSeconds) + "s");
    } else {
...
void testTask() {
  ...
  server.on("/setAccelerometerAxis", handleSetAccelerometerAxis);
  server.on("/setWiFi", handleSetWiFi);
  server.on("/setMQTT", handleSetMQTT);
  server.on("/setOptimizer", handleSetOptimizer);
  server.on("/setEspNow", handleSetEspNow);

...
<!DOCTYPE HTML><html>
  ...
  <table>
    <tr>
      ...
      <td>
      <h4 style="color:DodgerBlue;">%value% C </h4> 
      </td>
...
String processor(const String& var) {
  if (var == "ssidString") {
    ...
    return String(hum,3) + "% / " + String(temp,3) + " C";
  } else if (var == "dato") {
    int vol = voltage();
    return String(vol);
  }
...
void init_running_2(){
  ...
  if(status){ /* RPI RUNNING */
    ...
    if(!digitalRead(RPI_STATUS) || continuous){
      // MANAGER WHITE LOW TO CONINUE RUNNING
      rtc_gpio_init(RPI_MANAGER);
      rtc_gpio_set_direction(RPI_MANAGER, RTC_GPIO_MODE_OUTPUT_ONLY);
      rtc_gpio_hold_dis(RPI_MANAGER);
  ...
  JsonDocument docImg;
  DeserializationError error = deserializeJson(docImg, msgImg);
  ...
  const char* img = docImg["img"];
  const char* img_file = docImg["img_file"];
...
void rpiRequest(){
  ...
  JsonDocument docImg;
  DeserializationError error = deserializeJson(docImg, payload);
  ...
  String payloadM;
  serializeJson(doc, payloadM);
  
  //Serial.println(payload);
  client.publish(topicPublish, payloadM.c_str());
